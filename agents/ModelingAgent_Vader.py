...
# Class-level statistics tracker
_regeneration_stats_tracker = {}
...
class PlanningAndReasoningSimulationAgent(SimulationAgent):
    ...
    def __init__(self, llm: LLMBase):
        ...
        self.sentiment_threshold = 0.32
        # Track regeneration statistics
        # Use instance ID to track stats per agent instance
        ...
        self.regeneration_stats = {
            ...
            'regenerated_sentiment_scores': [],
            'sentiment_improvements': []
        }
        _regeneration_stats_tracker[self._instance_id] = self.regeneration_stats
    ...
    def workflow(self) -> Dict[str, Any]:
        ...
        # Track statistics at workflow level to ensure all reviews are counted
        self.regeneration_stats['total_reviews'] += 1
        _regeneration_stats_tracker[self._instance_id] = self.regeneration_stats
    ...
    def _compose_review_with_reasoning(self, context: Dict[str, Any]) -> str:
        ...
        try:
            ...
            original_review_text = review_text  # Store original for comparison
            original_sentiment_norm = self._sentiment_score(review_text)[1]

            # Option B: regenerate if sentiment and rating disagree
            mismatch, sent_norm = self._sentiment_mismatch(final_rating, review_text)
            if mismatch:
                self.regeneration_stats['regenerations_triggered'] += 1
                logger.info(
                    f"Sentiment mismatch detected: rating={final_rating:.1f}, sentiment={sent_norm:.3f}, "
                    f"diff={abs(sent_norm - (final_rating / 5.0)):.3f} (threshold={self.sentiment_threshold})"
                )
                logger.info(f"Original review (first 100 chars): {original_review_text[:100]}...")
                ...
                if parsed_regen:
                    ...
                    self.regeneration_stats['regenerations_successful'] += 1
                else:
                ...
                # Compare original vs regenerated (recommendation 3)
                regenerated_sentiment_norm = self._sentiment_score(review_text)[1]
                rating_norm = final_rating / 5.0
                original_diff = abs(original_sentiment_norm - rating_norm)
                regenerated_diff = abs(regenerated_sentiment_norm - rating_norm)
                improvement = original_diff - regenerated_diff
                
                self.regeneration_stats['original_sentiment_scores'].append(original_sentiment_norm)
                self.regeneration_stats['regenerated_sentiment_scores'].append(regenerated_sentiment_norm)
                self.regeneration_stats['sentiment_improvements'].append(improvement)
                _regeneration_stats_tracker[self._instance_id] = self.regeneration_stats
                
                logger.info(
                    f"Regeneration result: original_sentiment={original_sentiment_norm:.3f}, "
                    f"regenerated_sentiment={regenerated_sentiment_norm:.3f}, "
                    f"improvement={improvement:+.3f} (positive=better alignment)"
                )
                logger.info(f"Regenerated review (first 100 chars): {review_text[:100]}...")
...
if __name__ == "__main__":
    ...
    results_dir = Path(f"./VADER_{task_set}_{num_tasks}_tasks_{timestamp}")
    ...
    for name, agent_cls in strategies:
        ...
        # Clear regeneration stats tracker for new strategy run
        if name == "PlanningAndReasoning":
            _regeneration_stats_tracker.clear()
        ...
        try:
            ...
            # Print regeneration statistics if this is the VADER agent (recommendation 2)
            if name == "PlanningAndReasoning" and _regeneration_stats_tracker:
                # Aggregate stats from all agent instances
                aggregated_stats = {
                    'total_reviews': 0,
                    'regenerations_triggered': 0,
                    'regenerations_successful': 0,
                ...
                num_instances = len(_regeneration_stats_tracker)
                logger.info(f"Aggregating stats from {num_instances} agent instance(s)")
                for instance_id, instance_stats in _regeneration_stats_tracker.items():
                    logger.debug(f"Instance {instance_id}: {instance_stats['total_reviews']} reviews")
                    aggregated_stats['total_reviews'] += instance_stats['total_reviews']
                    aggregated_stats['regenerations_triggered'] += instance_stats['regenerations_triggered']
                    aggregated_stats['regenerations_successful'] += instance_stats['regenerations_successful']
                    aggregated_stats['original_sentiment_scores'].extend(instance_stats['original_sentiment_scores'])
                    aggregated_stats['regenerated_sentiment_scores'].extend(instance_stats['regenerated_sentiment_scores'])
                    aggregated_stats['sentiment_improvements'].extend(instance_stats['sentiment_improvements'])
                
                stats = aggregated_stats
                logger.info(f"Aggregated total: {stats['total_reviews']} reviews across {num_instances} instance(s)")
                if stats and stats['total_reviews'] > 0:
                    print(f"\n   VADER Regeneration Statistics:")
                    print(f"   Total reviews: {stats['total_reviews']}")
                    print(f"   Regenerations triggered: {stats['regenerations_triggered']} ({stats['regenerations_triggered']/max(1, stats['total_reviews'])*100:.1f}%)")
                    print(f"   Regenerations successful: {stats['regenerations_successful']}")
                    if stats['sentiment_improvements']:
                        avg_improvement = sum(stats['sentiment_improvements']) / len(stats['sentiment_improvements'])
                        positive_improvements = sum(1 for imp in stats['sentiment_improvements'] if imp > 0)
                        print(f"   Average sentiment improvement: {avg_improvement:+.3f}")
                        print(f"   Regenerations that improved alignment: {positive_improvements}/{len(stats['sentiment_improvements'])} ({positive_improvements/len(stats['sentiment_improvements'])*100:.1f}%)")
                    
                    # Save regeneration stats to file
                    stats_path = results_dir / f"{name}_regeneration_stats.json"
                    stats_to_save = {
                        'total_reviews': stats['total_reviews'],
                        'regenerations_triggered': stats['regenerations_triggered'],
                        'regenerations_successful': stats['regenerations_successful'],
                        'regeneration_rate': stats['regenerations_triggered'] / max(1, stats['total_reviews']),
                        'avg_sentiment_improvement': sum(stats['sentiment_improvements']) / len(stats['sentiment_improvements']) if stats['sentiment_improvements'] else 0,
                        'improvements_positive': sum(1 for imp in stats['sentiment_improvements'] if imp > 0),
                        'improvements_total': len(stats['sentiment_improvements']),
                    }
                    stats_path.write_text(json.dumps(stats_to_save, indent=4))
                    print(f"   Regeneration stats saved to {stats_path}")
