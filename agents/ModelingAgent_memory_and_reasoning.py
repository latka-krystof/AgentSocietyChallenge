...
from websocietysimulator.agent.modules.reasoning_modules import ReasoningCOTWithReflection, ReasoningBase
from websocietysimulator.agent.modules.memory_modules import MemoryUserProfile, MemoryDILU
import logging
...
class MySimulationAgentBaseline(SimulationAgent):
    """Baseline agent - original implementation."""
...
class MySimulationAgentMemoryAndReasoning(SimulationAgent):
    """
    Combined agent using:
    - MemoryUserProfile (best memory strategy)
    - ReasoningCOTWithReflection (best reasoning strategy)
    """
    
    def __init__(self, llm: LLMBase):
        """Initialize MySimulationAgentMemoryAndReasoning"""
        super().__init__(llm=llm)
        self.planning = PlanningBaseline(llm=self.llm)
...
if __name__ == "__main__":
    ...
    results_dir = f"./combined_comparison_{task_set}_{timestamp}"
    ...
    strategies = [
        ("Baseline", MySimulationAgentBaseline),
        ("MemoryAndReasoning", MySimulationAgentMemoryAndReasoning),  # UserProfile + COTWithReflection
    ]
    ...
    print("-" * 60)
    try:
        with open(os.path.join(results_dir, "Baseline_evaluation.json"), 'r') as f:
            baseline_eval = json.load(f)
        with open(os.path.join(results_dir, "MemoryAndReasoning_evaluation.json"), 'r') as f:
            combined_eval = json.load(f)
        
        if 'metrics' in baseline_eval and 'metrics' in combined_eval:
            baseline_metrics = baseline_eval['metrics']
            combined_metrics = combined_eval['metrics']
            ...
            print(f"\nMemoryAndReasoning (UserProfile + COTWithReflection):")
            print(f"  Preference Estimation: {combined_metrics.get('preference_estimation', 'N/A')}")
            print(f"  Review Generation: {combined_metrics.get('review_generation', 'N/A')}")
            print(f"  Overall Quality: {combined_metrics.get('overall_quality', 'N/A')}")
